...
from src.blacklist_utils import BlacklistServerClass, User, guest_user
from src.blacklist_utils import USER_TYPE_ADMIN, USER_TYPE_GUEST, USER_TYPE_KING
from src.config import Config, DEFAULT_AUTH_CODE, DEFAULT_MAIL
...
async def lifespan(_: FastAPI):
    '''fastapi的lifespan，用于在控制台接收到终止时关闭模拟Redis客户端(实际Redis客户端无需调用QUIT)'''
    yield
    redis_client.quit()
...
async def custom_rate_limit_handler(_: Request, __: RateLimitExceeded):
    '''自定义限流返回'''
    return restful(guest_user, 429, "服务器繁忙")
...
def decode_jwt(session_id) -> User:
    '''解码JWT'''
    try:
        decoded = jwt.decode(session_id, blacklist_server.jwt_secret, algorithms=['HS256'])
        if int(time.time()) > int(decoded['e']): # 过期
            return None
        result = User(qq_id=decoded['q'],
            unique_id=decoded['u'],
            type=decoded['t'],
            nickname=decoded['n'],
            password="")
...
def encode_jwt(user: User) -> str:
    ...
    payload = {'q': user.qq_id,
        'u': user.unique_id,
        't': user.type,
        'n': user.nickname,
        'e': expire_timestamp}
    result = jwt.encode(payload, blacklist_server.jwt_secret, algorithm='HS256')
    return result
...
def refresh_session(user: User, response: Response, remember_me: bool = False) -> Response:
    ...
    if user.session_expired: # 重新生成JWT令牌
        session_id = encode_jwt(blacklist_server.get_user(user.qq_id))
        response.set_cookie(key='session_id',
            value=session_id,
            httponly=True,
            secure=config.IS_HTTPS,
            samesite='strict',
            max_age=None) # 浏览器关闭后自动删除

    def create_remember_token(qq_id):
        '''生成记住密码的token'''
    ...
    if remember_me: # 只有在remember_me为True也就是登录时勾选了自动登录才会创建remember_token
        remember_token, token_hash = create_remember_token(user.qq_id)
        response.set_cookie(key='remember_token',
            value=remember_token,
            httponly=True,
            secure=config.IS_HTTPS,
            samesite='strict',
            max_age=REMEMBER_ME_TIME) # 设置httponly=False，可以用JavaScript读取
        response.set_cookie(key='hashed_token',
            value=token_hash,
            httponly=True,
            secure=config.IS_HTTPS,
            samesite='strict',
...
def get_current_user(request: Request) -> User:
    ...
    if session_id:
        user = decode_jwt(session_id.encode())
        if user:
            return user

    def verify_remember_token(token_hash, remember_token) -> int | None:
        '''验证记住密码的token'''
        if token_hash and remember_token and bcrypt.checkpw(
                remember_token.encode(), token_hash.encode()):
...
async def login(request: Request, form: LoginForm):
    ...
    try:
        ...
        if blacklist_server.login(qq_id, form.password.get_secret_value()):
            ...
            logger.info("login SUCCESS from IP: %s", request.client.host)
            return restful(user, 200, "登录成功", None, form.remember_me)
        return restful(guest_user, 401, "无效的用户名或密码")
    except Exception:
        logger.error("login ERROR from IP: %s", request.client.host)
        return restful(guest_user, 401, "无效的用户名或密码")
...
async def bapi_send_verify_code(request: Request, user: User = Depends(get_current_user)):
    """发送验证码邮件接口"""
    # 验证邮箱格式
    ...
    try:
        ...
        return restful(guest_user, 400, '请使用QQ邮箱注册')
    ...
    # 检查总发送频率
    rest_time = redis_client.ttl("vcode_limit:server")
    if rest_time != -2:
    ...
    redis_client.setex("vcode_limit:server", SERVER_RESEND_TIME, 1) # 限制服务器请求发送频率

    try:
        message = f"您正在<u>黑名单系统</u>注册账号或修改资料，您的验证码是: <b>{verification_code}</b>。\
验证码{EXPIRE_TIME//60}分钟有效，输错超过{MAX_VERIFY_FAIL_COUNT}次将失效，请勿泄露给他人。"
        if config.DEBUG_DO_NOT_SEND_EMAIL:
            logger.info("[config.DEBUG_DO_NOT_SEND_EMAIL] 验证码：%s (%s <- %s)",
                verification_code,
                recv_email,
                message)
        ...
        try:
            ...
            logger.error("SMTP服务器响应异常： %s", e) # 由于部分邮箱如QQ邮箱不能很好的对接smtplib，即使发送成功也会有异常
...
async def bapi_check_email_type(recv_email: str):
    """检查邮箱类型"""
    ...
    try:
        ...
        return restful(guest_user, 400, "请使用旧版QQ邮箱注册")
    try:
        type_id, type_name = blacklist_server.get_register_type(qq_id)
        has_password = blacklist_server.check_exist_user(qq_id)
        return restful(guest_user, 200, "邮箱类型检查成功",
            {'type': type_id,
            'type_name': type_name,
...
async def bapi_verify(request: Request, user: User = Depends(get_current_user)):
    """修改个人信息接口（需要邮箱验证码）"""
    item = await request.json()
    recv_email = item.get('email', '')
    qq_id = None
    try:
        ...
        return restful(guest_user, 400, "请使用旧版QQ邮箱注册")
    is_admin = user.type == USER_TYPE_ADMIN if user.qq_id != qq_id else False
        # 安全起见，管理员修改自己信息需要验证码，且不允许修改自己的类型
    ...
    old_user_type, _ = blacklist_server.get_register_type(qq_id)
    ...
    try:
        ...
        if password:# 刷新session（仅当密码修改时）
            blacklist_server.refresh_jwt_secret()
        else: # 没有修改密码，刷新JWT
            user.session_expired = True
        # 清除remember_token
...
async def bapi_search(request: Request, user: User = Depends(get_current_user)):
    """搜索黑名单接口"""
    try:
        item = await request.json()
        start = time.time()
...
async def bapi_update(request: Request, user: User = Depends(get_current_user)):
    """添加、修改黑名单接口"""
    item = await request.json()
    if user.type == USER_TYPE_GUEST:
        return restful(user, 403, '请先登录')
    ...
    except OSError:# 给出了负数时间戳
        return restful(user, 400, '创建日期不能是负数')
    except (ValueError, TypeError):
    ...
    try:
        ...
        return restful(user, 400, 'table_id必须为数字')
    except AssertionError:
    ...
    try:
        ...
        return restful(user, 400, '退班日期不能是负数')
    except (ValueError, TypeError):
...
async def bapi_delete(request: Request, user: User = Depends(get_current_user)):
    '''删除黑名单记录接口'''
    item = await request.json()
    if user.type == USER_TYPE_GUEST:
        return restful(user, 403, '请先登录')
    ...
    try:
        ...
        return restful(user, 400, 'create_time不能是负数')
    except (ValueError, TypeError):
    ...
    try:
        ...
        logger.info("删除黑名单成功: uid=%s, create_time=%s。操作者：%s(%s)类型[%s]",
            uid,
            create_time,
            user.nickname,
            operator_qq_id,
...
async def bapi_my_info(user: User = Depends(get_current_user)):
    '''获取当前用户信息'''
    avatar = None
    if config.DEBUG_AVATAR_QQ_URL and user.qq_id and user.qq_id > 0:
        avatar = f"https://q1.qlogo.cn/g?b=qq&nk={user.qq_id}&s=640"
    else:
        avatar = f'/static/img/{user.type}.jpg'
    return {
        "avatar": avatar,
        "qq_id": user.qq_id,
...
async def bapi_user_info(request: Request, user: User = Depends(get_current_user)):
    '''获取用户信息'''
    item = await request.json()
    try:
...
@app.get('/bapi/config')
async def bapi_config(user: User = Depends(get_current_user)):
    '''获取配置信息'''
...
async def bapi_update_config(request: Request, user: User = Depends(get_current_user)):
    '''更新配置信息'''
    item = await request.json()
    if user.type != USER_TYPE_ADMIN:
        return restful(user, 403, '权限不足')
...
async def bapi_latest_log(request: Request, user: User = Depends(get_current_user)):
    '''获取最新日志'''
    item = await request.json()
    # 非管理员展示最近10条日志
    try:
...
@app.post('/oapi/bulk')
async def bapi_bulk_add_user(request: Request, user: User = Depends(get_current_user)):
    '''批量添加用户'''
    item = await request.json()
    if user.type != USER_TYPE_ADMIN:
        return restful(user, 403, '权限不足')
...
@app.get('/oapi/count')
async def bapi_count(user: User = Depends(get_current_user)):
...
@app.get('/oapi/repair')
async def bapi_repair_blacklist(user: User = Depends(get_current_user)):
    '''修复黑名单'''
    if user.type != USER_TYPE_ADMIN:
        return restful(user, 403, '权限不足')
    try:
...
async def blacklist(user: User = Depends(get_current_user)):
    '''下载黑名单数据库（用于备份）'''
    if user.type != USER_TYPE_ADMIN:
        return restful(user, 403, '权限不足')
    try:
...
async def bapi_upload(request: Request, user: User = Depends(get_current_user)):
    ...
    try:
        ...
        reload()
        return restful(user, 200, '上传成功，数据库重载成功')
    except Exception as e:
        return restful(user, 400, f'{e}')

@app.get('/oapi/reload')
async def bapi_reload(user: User = Depends(get_current_user)):
    '''重新加载黑名单数据库'''
    if user.type != USER_TYPE_ADMIN:
        return restful(user, 403, '权限不足')
    try:
...
def restful(user: User, code: int, msg: str = '', data: dict = None, rmb: bool = False) -> Response:
    '''以RESTful的方式进行返回响应'''
    retcode = 1
    if code == 200:
        retcode = 0
